...
export function OrderDetailsTab({
  ...
  const [dayFilters, setDayFilters] = useState<Record<string, PaymentFilter>>(
    {},
  );
  ...
  const setDayFilter = (date: string, filter: PaymentFilter) => {
    setDayFilters((prev) => ({ ...prev, [date]: filter }));
  };
  ...
  const filterButtons: { label: string; value: PaymentFilter }[] = [
    { label: "All", value: "all" },
      ...
      <div className="px-4 pt-5 pb-3 flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {orders.length} total bill{orders.length !== 1 ? "s" : ""}
          </p>
        </div>
        <button
          type="button"
      ...
      {orders.length === 0 ? (
        ...
        <div className="px-4 pb-6 space-y-4">
          {dayGroups.map((group) => {
            ...
            const activeDayFilter = dayFilters[group.date] ?? "all";

            // Always unfiltered totals for chips and collapsed summary
            const cashOrders = group.orders.filter(
              (o) => o.paymentType === "cash",
            );
            const onlineOrders = group.orders.filter(
              (o) => o.paymentType === "online",
            );
            const dayCash = cashOrders.reduce((s, o) => s + o.total, 0);
            const dayOnline = onlineOrders.reduce((s, o) => s + o.total, 0);
            const dayCashCount = cashOrders.length;
            const dayOnlineCount = onlineOrders.length;
            const dayTotal = group.orders.reduce((s, o) => s + o.total, 0);

            // Filtered orders only used inside expanded view
            const filteredDayOrders =
              activeDayFilter === "all"
                ? group.orders
                : group.orders.filter((o) => o.paymentType === activeDayFilter);
            const filteredDayTotal = filteredDayOrders.reduce(
              (s, o) => s + o.total,
              0,
            );
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-3 px-4 py-3 bg-secondary border-b border-border">
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      ...
                      {/* Bills + total (always unfiltered in header) */}
                      <p className="text-xs text-muted-foreground mt-0.5">
                        {group.orders.length} bill
                        {group.orders.length !== 1 ? "s" : ""} · ₹
                        {dayTotal.toLocaleString("en-IN")}
                      </p>
                      {/* Cash + Online breakdown (always unfiltered) */}
                      <div className="flex items-center gap-2 mt-1">
                        <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-green-700 bg-green-50 border border-green-200 rounded-full px-2 py-0.5">
                          💵 ₹{dayCash.toLocaleString("en-IN")} ({dayCashCount})
                        </span>
                ...
                {/* Orders for this day (expanded) */}
                {isExpanded && (
                  <div className="divide-y divide-border">
                    {/* Filter bar inside expanded section */}
                    <div className="px-4 py-2.5 flex items-center gap-2 bg-background border-b border-border/60">
                      <span className="text-xs text-muted-foreground font-medium">
                        Filter:
                      </span>
                      <div className="flex items-center rounded-lg border border-border bg-secondary overflow-hidden text-xs font-semibold">
                        {filterButtons.map((btn) => (
                          <button
                            key={btn.value}
                            type="button"
                            onClick={() => setDayFilter(group.date, btn.value)}
                      ...
                      <span className="text-xs text-muted-foreground ml-auto">
                        {filteredDayOrders.length} bill
                        {filteredDayOrders.length !== 1 ? "s" : ""}
                        {activeDayFilter !== "all" && (
                          <span className="ml-1">
                            · ₹{filteredDayTotal.toLocaleString("en-IN")}
                          </span>
                        )}
                      </span>
                    </div>
                    ...
                    {filteredDayOrders.length === 0 ? (
                      <div className="px-4 py-4 text-center">
                        <p className="text-xs text-muted-foreground">
                          No {activeDayFilter} orders for this day.
                        </p>
                      </div>
                    ) : (
